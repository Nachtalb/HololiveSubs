...
def add_event(calendar: Calendar, new_event: Event, old_event_id):
    copy = calendar.events.copy()
    for event in calendar.events:
        if event.uid == old_event_id:
            copy.remove(event)
...
for group in stats.values():
    for member in group["members"]:
        ...
        if member["video"]:
            ...
            add_event(calendar, event, old_event_id)
            all_events.append((event, old_event_id))
        elif is_new:
            todo = Todo(
...
for event, old_event_id in all_events:
    add_event(calendar, event, old_event_id)
